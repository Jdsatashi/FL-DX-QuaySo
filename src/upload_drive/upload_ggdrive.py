...
from src.utils.env import FOLDER_ID
...
def upload_to_drive(msg_filename, applog_filename, log_folder_path):
    ...
    file_app_log = drive.CreateFile({'parents': [{'id': FOLDER_ID}], 'title': app_log})
    ...
    file_msg_log = drive.CreateFile({'parents': [{'id': FOLDER_ID}], 'title': msg_log})
